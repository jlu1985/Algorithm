...
/**
 * <p>In computer science, counting sort is an algorithm for sorting a collection
 ...
 * algorithm, radix sort, that can handle larger keys more
 * efficiently.Because counting sort uses key values as indexes into an
 * array, it is not a comparison sort, and the O(n log n) lower bound for
 * comparison sorting does not apply to it. Bucket sort may be used for many
 * of the same tasks as counting sort, with a similar time analysis; however,
 * compared to counting sort, bucket sort requires linked lists, dynamic arrays
 * or a large amount of preallocated memory to hold the sets of items within
 * each bucket, whereas counting sort instead stores a single number (the count
 * of items) per bucket.</p>
 * 
 * @author Jia Lu
